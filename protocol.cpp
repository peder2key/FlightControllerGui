...
Protocol::Protocol()
{
    ...
    mSetupSerial = new setupSerial();

    connect(serialPort,SIGNAL(readyRead()),this,SLOT(ReadFrame()));
    connect(mSetupSerial, SIGNAL(portBaudSetting(QString,qint32)), this, SLOT(Connect(QString,qint32)));
...
void Protocol::showSerialSetup()
{

    mSetupSerial->GetPortInfo();
    mSetupSerial->show();
}
bool Protocol::Connect(QString port, qint32 baud)
{
   qDebug() << "Connected to:" << port << baud;
